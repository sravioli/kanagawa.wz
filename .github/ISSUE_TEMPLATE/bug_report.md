...
- **kanagawa.wz version/commit**: (e.g. `v1.0.0` or commit hash)
- **Installation method**: git (`wezterm.plugin.require`) / local checkout
- **Lua version** (if known):

## Setup

Provide your kanagawa.wz configuration and usage code so the issue can be
reproduced.

```lua
-- Example:
-- kanagawa.apply_to_config(config, { scheme = "wave" })
-- local colors = kanagawa.get("dragon", { background = "#000000" })
```
